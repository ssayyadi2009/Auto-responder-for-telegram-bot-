...
def webhook():
    ...
    if text == "/start":
        ...
        if not info:
            # اولین بار
            shuffled = NEXT_MESSAGES.copy()
            random.shuffle(shuffled)
            user_data[chat_id] = {
                "queue": shuffled,  # صف پیام‌ها
                "used": []
            }
            ...
            if not queue:
                # همه پیام‌ها استفاده شده → ریست و shuffle دوباره
                queue = NEXT_MESSAGES.copy()
                random.shuffle(queue)
                used = []

            # پیام بعدی را از ابتدای صف بردار
            next_msg = queue.pop(0)
            used.append(next_msg)
            send_message(chat_id, next_msg)

            # بروزرسانی
            user_data[chat_id]["queue"] = queue
            user_data[chat_id]["used"] = used
